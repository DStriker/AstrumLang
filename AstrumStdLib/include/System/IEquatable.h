...
namespace System {
	...
	template<class __AnyType> concept __HasMethodImplementation_11c3fd746814643f = requires(typename __AnyType::__class t) { {t.operator==(std::declval<typename __AnyType::__self>())} -> std::convertible_to<bool>; }  || requires(typename __AnyType::__self t) { {operator==(__extensions::__proxy<typename __AnyType::__self>{t}, std::declval<typename __AnyType::__self>())} -> std::convertible_to<bool>; } || requires { std::declval<typename __AnyType::__self&>() ==std::declval<typename __AnyType::__self>(); };
	namespace __vtables {
