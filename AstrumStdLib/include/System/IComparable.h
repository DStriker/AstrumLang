...
namespace System {
	...
	template<class __AnyType> concept __HasMethodImplementation_199d3b21e4765375 = requires(typename __AnyType::__class t) { {t.operator<=>(std::declval<typename __AnyType::__self>())} -> std::convertible_to<Builtin::i32>; }  || requires(typename __AnyType::__self t) { {operator<=>(__extensions::__proxy<typename __AnyType::__self>{t}, std::declval<typename __AnyType::__self>())} -> std::convertible_to<Builtin::i32>; } || requires { std::declval<typename __AnyType::__self&>() <=>std::declval<typename __AnyType::__self>(); };
	namespace __vtables {
