...
namespace System::Numerics {
	...
	template<class __AnyType, class TRight, class TResult> concept __HasMethodImplementation_7a1425cf208e27c4 = requires(typename __AnyType::__class t) { {t.operator%(std::declval<TRight>())} -> std::convertible_to<TResult>; }  || requires(typename __AnyType::__self t) { {operator%(__extensions::__proxy<typename __AnyType::__self>{t}, std::declval<TRight>())} -> std::convertible_to<TResult>; } || requires { std::declval<typename __AnyType::__self&>() %std::declval<TRight>(); };
	namespace __vtables {
