...
namespace System::Numerics {
	...
	template<class __AnyType> concept __HasMethodImplementation_5de63977afaaf100 = requires(typename __AnyType::__class t) { {t.operator+(std::declval<typename __AnyType::__self>())} -> std::convertible_to<typename __AnyType::__self>; }  || requires(typename __AnyType::__self t) { {operator+(__extensions::__proxy<typename __AnyType::__self>{t}, std::declval<typename __AnyType::__self>())} -> std::convertible_to<typename __AnyType::__self>; } || requires { std::declval<typename __AnyType::__self&>() +std::declval<typename __AnyType::__self>(); };
	#line 9 "C:\\Users\\user\\Documents\\VSProjects\\MyLanguage\\AstrumStdLib\\include\\System\\Numerics\\IAdditionOperators.ast"
	template<class __AnyType> concept __HasMethodImplementation_13c5b9f59c19ffab = requires(typename __AnyType::__class t) { {t.operator+=(std::declval<typename __AnyType::__self>())} -> std::convertible_to<typename __AnyType::__self>; }  || requires(typename __AnyType::__self t) { {operator+=(__extensions::__proxy<typename __AnyType::__self>{t}, std::declval<typename __AnyType::__self>())} -> std::convertible_to<typename __AnyType::__self>; } || requires { std::declval<typename __AnyType::__self&>() +=std::declval<typename __AnyType::__self>(); };
	namespace __vtables {
